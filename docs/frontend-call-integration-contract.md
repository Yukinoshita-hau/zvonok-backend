...
| Leave call | `/app/call/leave` | `LeaveCallDto` | `callId` **or** `chatRoomId` fallback | Yes | Yes | PRIVATE leave ends call for both; GROUP member leaves self, session ends only when no active participants remain |
...
| `CALL_PARTICIPANT_LEFT` | All call participants | On leave action | Update participant presence only; do not treat as global end by itself | Yes (followed by `CALL_ENDED`) | Yes | `participantStatus=LEFT`, `callId` |

### Events declared but **not emitted now** (important for frontend)
- `CALL_END`, `CALL_CANCELLED`, `GROUP_CALL_STARTED`, `CALL_BUSY` currently есть в enum, но в `CallSessionService` не публикуются.
...
- Host red button: `/app/call/end` -> `CALL_ENDED` всем participants (global end).
- Non-host red button: `/app/call/leave` -> `CALL_PARTICIPANT_LEFT`; call continues if active participants remain.
- If last active participant leaves, backend ends session and emits `CALL_ENDED`.
...
- `sendDecline`: same.
- `sendEnd`: send `callId` (host in GROUP, either participant in PRIVATE).
- `sendLeave`: send `callId` for non-host GROUP leave (and optional PRIVATE leave shortcut).
...
6. Group non-host leave now has dedicated endpoint/event (`/app/call/leave`, `CALL_PARTICIPANT_LEFT`).

---

## Known backend gaps affecting frontend (must be handled)

1. `CallType` enum содержит значения, которые пока не используются фактически.
2. `callType` из `InviteCallDto` не возвращается в `BaseCallEvent`.
